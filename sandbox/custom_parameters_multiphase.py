...
from bioptim import (
    ...
    PenaltyController,
    ObjectiveList,
    PhaseDynamics, DynamicsList,
)


def my_parameter_function(bio_model: BiorbdModel, value: MX, extra_value: Any, **extra_parameters):
    """
...
def prepare_ocp(
    ...
    for i in range(n_phase):
        ...
        # x_bounds[i]["q"][:, [0, -1]] = 0
        # x_bounds[i]["q"][1, -1] = 3.14
        # x_bounds[i]["qdot"][:, [0, -1]] = 0

    # Define control path constraint
    tau_min, tau_max = -300, 300
    u_bounds = BoundsList()
    for i in range(n_phase):
        u_bounds.add("tau", bounds=None, min_bound=[tau_min] * n_tau, max_bound=[tau_max] * n_tau, phase=i)
        # u_bounds[i]["tau"][1, :] = 0
    ...
    if optim_gravity:
        ...
        for i in range(n_phase):
            ...
            parameter_bounds.add("gravity_xyz",
                                 min_bound=min_g,
                                 max_bound=max_g,
                                 interpolation=InterpolationType.CONSTANT,
                                 phase=i)
            # parameter_bounds[i]['gravity_xyz'][:, [0]] = 0

        # and an initial condition
        # parameter_init[0]["gravity_xyz"] = (min_g + max_g) / 2
